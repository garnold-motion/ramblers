...
const Specials = ({ setActiveTab, beers = [], setSelectedBeer }) => {
  ...
  const {
    data: specialsData,
    isLoading,
  } = useSheetData(SHEET_URLS.specials, 'ramblers_specials_v1', transformSpecials);

  const todayIndex = new Date().getDay();
  const featuredBeer = beers.find(b => b.is_featured === 'TRUE') || beers[0];

  // TRICK 2: useLayoutEffect. Runs synchronously BEFORE the browser paints.
  useLayoutEffect(() => {
    if (specialsData.length > 0 && scrollRef.current && !hasSnapped.current) {
  ...
  if (isLoading && specialsData.length === 0) {
    return <div className="flex justify-center pt-20 text-brand-org-1 animate-pulse font-bold tracking-widest text-xs uppercase">Loading...</div>;
  }

  const nameLength = featuredBeer?.name?.length || 0;
  ...
  return (
    <div className="flex flex-col lg:flex-row landscape:flex-row h-full w-full bg-[#1a1a1a] overflow-hidden">
      ...
      <div className="h-1/2 lg:h-full landscape:h-full w-full lg:w-1/2 landscape:w-1/2 flex flex-col justify-center items-center px-6 py-4 lg:p-10 landscape:p-10 min-h-0 shrink-0">
        <div
        ...
        >
          ...
          <div className="flex-1 pl-6 sm:pl-10 z-10 flex flex-col justify-center text-left min-w-0 py-2">
            ...
            <h2 className={`
              ${nameLength > 12 ? 'text-xl sm:text-2xl leading-[1]' : 'text-3xl sm:text-4xl leading-[0.9]'}
              font-black text-white uppercase tracking-tighter mb-1.5 shrink-0
              flex-wrap break-words
            `}>
              {featuredBeer ? featuredBeer.name : 'Loading...'}
            </h2>

            <p className="text-brand-org-1 text-[10px] sm:text-sm font-black uppercase tracking-widest mb-2 truncate shrink-0">
              Tap {featuredBeer?.tap_number || '1'} · {featuredBeer?.style || 'Lager'}
            </p>
            ...
            <div className="flex items-center gap-2 text-white/40 text-[10px] sm:text-xs font-black uppercase tracking-widest group-hover:text-brand-org-1 transition-colors shrink-0">
